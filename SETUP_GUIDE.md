# Setup Guide for Antigravity

This file is a step-by-step guide for Antigravity (or any LLM agent) to install and configure the TradingView MCP server. If a user asks you to install or set up this project, follow these steps exactly.

## Step 1: Clone and Install

```bash
git clone https://github.com/cucarachatraicionera/tradingview-mcp.git ~/tradingview-mcp
cd ~/tradingview-mcp
npm install
```
...
Add the server to Antigravity's MCP configuration:
...
Replace `<INSTALL_PATH>` with the actual path where the repo was cloned (e.g., `/home/user/tradingview-mcp`).
...
The MCP server only loads when Antigravity starts. After adding the config:
...
| MCP server not showing | Check MCP config syntax, restart Antigravity |
...
- `ANTIGRAVITY.md` — Decision tree for which tool to use when
